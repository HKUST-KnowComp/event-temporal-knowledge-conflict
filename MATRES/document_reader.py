...
def id_lookup(span_SENT, start_char):
    ...
    return token_id
'''
def subword_id_getter(sent, e_start_char, transformers_model):
    """
    Example: "sent": "Bob hit Jacob.",
             "e_start_char": 4
    Output: ([0, 3045, 478, 5747, 4, 2], 2)
    """
    _subword_to_ID, _subwords, _subword_span, _ = transformers_list(sent, transformers_model)
    #return _subword_to_ID, id_lookup(_subword_span, e_start_char)  # 0104_3.pt
    return _subword_to_ID, id_lookup(_subword_span, e_start_char) + 1 # 0321_0.pt
...
def subword_id_getter_space_split(space_sep_sent, e_start_char):
    ...
    for i in range(len(space_split_tokens)):    
        start_char_to_subword_id[token_span[i][0]] = len(_subword_to_ID)
        _subword_to_ID.extend(tokenizer.encode(space_split_tokens[i])[1:-1])
    _subword_to_ID.append(2)
    try:
        subword_id = start_char_to_subword_id[e_start_char]
    except:
        raise Exception('The provided event start char is wrong')
    return _subword_to_ID, subword_id
'''
...
#       KAIROS Reader
# =========================
def ltf_reader(dir_name, file_name, transformers_model, spaCy = True):
    tokenizer = AutoTokenizer.from_pretrained(transformers_model)    
    my_dict = {}
...
#       HiEve Reader
# =========================
#def tsvx_reader(dataset, dir_name, file_name, transformers_model, eventseg = 1):
def tsvx_reader(dataset, dir_name, file_name, tokenizer, eventseg = 1):
    #tokenizer = AutoTokenizer.from_pretrained(transformers_model)    
    my_dict = {}
