...
import numpy as np
import json
from model import transformers_mlp_cons, PoeClassifier
...
def main():
    ...
    parser.add_argument("--bias_model_path", type=str, required=True,
                        default=None, help="Path to resume training from.")
    ...
    parser.add_argument("--poe_mode", default="poe", choices=["poe", "poe_mixin", "poe_mixin_h"],
                        type=str, help="poe mode")
    ...
    print("Loading the bias model from " + args.bias_model_path)
    bias_model_state_dict = torch.load(args.bias_model_path)

    poe_classifier = PoeClassifier(robust_model, bias_model, bias_model_state_dict,
                                   params, poe_mode=args.poe_mode, entropy_weight=args.entropy_weight)
    poe_classifier.to("cuda:0")
    ...
    if debug:
        params['epochs'] = 1
    mem_exp_test = exp(cuda, poe_classifier, params['epochs'], params['learning_rate'],
